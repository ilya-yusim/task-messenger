...
if [ -f "$SCRIPT_DIR/../worker/tm-worker" ]; then
    # Running from installation directory
    WORKER_BIN="$SCRIPT_DIR/../worker/tm-worker"
elif [ -f "$INSTALL_DIR/tm-worker/tm-worker" ]; then
    # Running from default installation
    WORKER_BIN="$INSTALL_DIR/tm-worker/tm-worker"
elif command -v tm-worker &> /dev/null; then
    # Worker is in PATH
