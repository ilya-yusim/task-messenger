...
if [ -f "$SCRIPT_DIR/../manager/manager" ]; then
    # Running from installation directory
    MANAGER_BIN="$SCRIPT_DIR/../manager/manager"
elif [ -f "$INSTALL_DIR/manager/manager" ]; then
    # Running from default installation
    MANAGER_BIN="$INSTALL_DIR/manager/manager"
elif command -v manager &> /dev/null; then
    # Manager is in PATH
    MANAGER_BIN="manager"
else
