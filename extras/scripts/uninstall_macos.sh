...
cleanup_empty_directories() {
    ...
    local install_base="$DEFAULT_INSTALL_BASE"
    local config_base="$CONFIG_BASE"
    
    # Remove TaskMessenger directories if empty
    if [ -d "$install_base" ] && [ -z "$(ls -A "$install_base")" ]; then
        rmdir "$install_base" 2>/dev/null || true
        print_info "Removed empty directory: $install_base"
    fi
    
    if [ -d "$(dirname "$config_base")" ]; then
        local tm_config_parent="$(dirname "$config_base")"
        if [ -d "$tm_config_parent" ] && [ -z "$(ls -A "$tm_config_parent")" ]; then
            rmdir "$tm_config_parent" 2>/dev/null || true
            print_info "Removed empty directory: $tm_config_parent"
        fi
    fi
}
...
remove_from_path_instruction() {
    ...
    print_info "from your ~/.zshrc or ~/.bash_profile file"
}
