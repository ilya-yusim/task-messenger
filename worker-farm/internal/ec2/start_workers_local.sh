...
# SSM/non-login shells may not define HOME. Under `set -u`, any $HOME
# expansion would fail unless we provide a stable fallback first.
if [[ -z "${HOME:-}" ]]; then
...
# non-login shell so `tm-worker` resolves without requiring absolute
# worker_bin paths in inventory.
case ":${PATH:-}:" in
    *":$HOME/.local/bin:"*) ;;
...
# When piped via `bash -s` (e.g. by the worker-farm controller over
# `gh codespace ssh`) BASH_SOURCE[0] is unset, so the script_dir/repo_root
# defaults below would explode under `set -u`. Fall back to PWD in that
# case; callers piping the script are expected to pass -b/-c explicitly,
# so the relative-to-repo defaults won't actually be used.
