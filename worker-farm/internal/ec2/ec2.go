// Package ec2 contains AWS EC2 host-management primitives for the
// worker-farm remote backend.
package ec2
...
func EnsureInstance(ctx context.Context, cfg inventory.EC2Cfg, hostID, controllerID string) (*Instance, error) {
	if strings.TrimSpace(hostID) == "" {
		return nil, errors.New("ec2.EnsureInstance: hostID is required")
	}
	if strings.TrimSpace(controllerID) == "" {
		return nil, errors.New("ec2.EnsureInstance: controllerID is required")
	}
...
// StatusValue is the status vocabulary for an EC2 host, matching the
// plan's status table in step 4.
...
// without mutating anything. It maps the raw AWS state to the status
// vocabulary defined in the plan.
func QueryStatus(ctx context.Context, cfg inventory.EC2Cfg, hostID, controllerID string) InstanceStatus {
...
// StopInstance requests a graceful EC2 stop for the instance.
func StopInstance(ctx context.Context, cfg inventory.EC2Cfg, instanceID string) error {
	if strings.TrimSpace(instanceID) == "" {
		return errors.New("ec2.StopInstance: instanceID is required")
	}
	cl, err := newClients(ctx, cfg.Region)
	if err != nil {
		return err
	}
	_, err = cl.ec2.StopInstances(ctx, &e2.StopInstancesInput{InstanceIds: []string{instanceID}})
	if err != nil {
		return fmt.Errorf("ec2 stop %s: %w", instanceID, err)
	}
	return nil
}

// TerminateInstance requests EC2 termination for the instance.
func TerminateInstance(ctx context.Context, cfg inventory.EC2Cfg, instanceID string) error {
	if strings.TrimSpace(instanceID) == "" {
		return errors.New("ec2.TerminateInstance: instanceID is required")
	}
	cl, err := newClients(ctx, cfg.Region)
	if err != nil {
		return err
...
func newClients(ctx context.Context, region string) (*clients, error) {
	...
	if region == "" {
		return nil, errors.New("ec2: region is required")
	}
...
func createManagedInstance(ctx context.Context, cli *e2.Client, cfg inventory.EC2Cfg, hostID, controllerID string) (*ec2types.Instance, error) {
	...
	if t := strings.TrimSpace(cfg.InstanceType); t != "" {
		input.InstanceType = ec2types.InstanceType(t)
	}
