...
func TestLoadEC2SnapshotHostValid(t *testing.T) {
	p := writeInventoryFile(t, `{
  "hosts": [
    {
      "id": "aws-snapshot-1",
	...
	h := inv.Hosts[0]
	if h.Backend != BackendEC2Snapshot {
		t.Fatalf("host backend = %q, want %q", h.Backend, BackendEC2Snapshot)
	}
	if h.EC2Snapshot == nil {
		t.Fatal("host.ec2_snapshot is nil, want populated config")
	}
	if h.EC2Snapshot.Region != "us-east-1" {
		t.Fatalf("host.ec2_snapshot.region = %q, want us-east-1", h.EC2Snapshot.Region)
	}
	if h.EC2Snapshot.LaunchTemplateID != "lt-0abc123" {
		t.Fatalf("host.ec2_snapshot.launch_template_id = %q, want lt-0abc123", h.EC2Snapshot.LaunchTemplateID)
	}
}

func TestLoadEC2SnapshotHostInvalid(t *testing.T) {
	tests := []struct {
		name       string
		inventory  string
		wantSubstr string
  ...
  "hosts": [
    {
      ...
      "ec2_snapshot": {
        "launch_template_id": "lt-0abc123"
			...
			wantSubstr: "hosts[0].ec2_snapshot.region",
		},
		{
			name: "bad launch template id format",
			inventory: `{
  "hosts": [
    {
      ...
      "ec2_snapshot": {
		...
		{
			name: "invalid current ami id prefix",
			inventory: `{
  "hosts": [
    {
      "id": "aws-snapshot-1",
      ...
      "ec2_snapshot": {
        "region": "us-east-1",
        "launch_template_id": "lt-0abc123",
        "current_ami_id": "snap-123"
      }
    }
  ]
}`,
			wantSubstr: "hosts[0].ec2_snapshot.current_ami_id",
		},
	}
